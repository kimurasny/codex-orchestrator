...
- `project_root`（解析対象リポジトリのルート）が相対パスの場合は **設定ファイル（`--config`）の配置ディレクトリ** を基準に解決します（`--config` 未指定時のみカレントディレクトリが基準）。**別リポジトリを解析する場合はその絶対パスを指定** します。targets.txt 内の相対パスはこの `project_root` を基準に存在チェック・解析されます。
- `targets_file` / `output_dir` / `templates_dir` / `status_file` / `log_file` は `project_root` ではなく **設定ファイルの配置ディレクトリ** を基準に解決します。これにより `project_root` に別リポジトリを指定しても、テンプレート・対象一覧・出力・ログはオーケストレーター側に留まります。
...
python run.py --project-root D:\git\other         # 解析対象リポジトリを指定
...
| `--project-root`, `-p` | 解析対象リポジトリのルート（YAML より優先。別リポジトリ解析時に指定） |
...
`project_root` に **解析対象リポジトリの絶対パス** を指定します。対象一覧（targets.txt）のパスはその `project_root` からの相対パスで記述します。テンプレート・出力先・ログはオーケストレーター側に留まるため、解析対象リポジトリを汚しません。

```bash
# 設定ファイルはそのまま、対象リポジトリだけ差し替える例
python run.py --config config/config.yaml ^
  --project-root D:\git\other-project ^
  --targets config\other-targets.txt
```

`config\other-targets.txt`（`D:\git\other-project` からの相対パス）:

```
src\BL\Common\LoginService.cs
...
- 区切り文字は `/`（POSIX）・`\`（Windows）のどちらでも可（内部で正規化）
- 空行は無視
- `#` から始まる行はコメントとして無視
- 重複は自動除外
- 存在しないファイルは警告を出しスキップ
