...
> `--config` を省略した場合は、`run.py` と同じ場所にある `config/config.yaml` を自動で読み込みます（実行時のカレントディレクトリに依存しません）。設定を反映するには、この `config/config.yaml` を編集するか `--config` で明示指定してください。

```bash
python run.py                                     # config/config.yaml を自動読み込みして実行
python run.py --config config.yaml                # 設定ファイルを明示指定して実行
...
| オプション | 説明 |
| --- | --- |
| `--config`, `-c` | YAML 設定ファイルのパス（省略時は `config/config.yaml` を自動読み込み） |
